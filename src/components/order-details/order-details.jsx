...
function OrderDetails({ order, isOrderPosting, isOrderError }) {
    return (
        <div className={styles.container}>
            ...
            {isOrderError && (
                <p
                    className={`text text_type_main-medium mt-30  ${styles.title}`}
                >
                    Ошибка при соединении.
                </p>
            )}

            {!isOrderError && (isOrderPosting ? (
            ...
            {!isOrderError && <div className={`mt-15 ${isOrderPosting && styles.blink}`}>
                <img src={done} className={styles.image} alt="изображение недоступно" />
            </div>}
